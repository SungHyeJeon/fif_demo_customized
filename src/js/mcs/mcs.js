 ...
 * Oracle Mobile Cloud Service JavaScript SDK for Cordova, Release: 16.3.3.0, E76063-04
*/
...
Logger.prototype.verbose = function(message){
  this.log(LOG_LEVEL.VERBOSE, message);
};

Logger.prototype.warn = function(message){
  this.log(LOG_LEVEL.ERROR, message, true);
};

Logger.prototype.log = function(level, message, isWarn) {
  if(this.logLevel >= level) {
    if(this.logLevel === LOG_LEVEL.ERROR){
      isWarn ? console.warn(message) : console.error(message);
    } else {
      console.log(message);
    }
  }
 ...
 * @global
 */
function BrowserPlatform(logger, utils) {
 ...
 * @global
 */
function CordovaPlatform(logger, utils) {
  logger = logger || mcs._logger;
  utils = utils || mcs._utils;
  BrowserPlatform.call(this, logger, utils);
  ...
  function authenticateInvoke (successCallback, errorCallback){
    ...
    function invoke(resolve, reject){
      ...
      browserRef.addEventListener('loadstart', function loadStart(event) {
        if ((event.url).indexOf(redirect_uri) === 0) {
          ...
          if (social_token.access_token !== undefined && social_token.access_token !== null) {
            ...
            if ((event.url).indexOf('error_code=100') !== 0 && !_isAuthorized) {
      ...
      browserRef.addEventListener('exit', function () {
        if (_this._getIsAuthorized() !== true || IsInAppBrowserInstalled(metadata) !== true) {
          reject(new NetworkResponse(100, 'Cannot authenticate via a web browser'));
        }
      })
    }
 ...
 * @constructor
 */
function MobileBackendManager(logger, utils, persistence) {
  ...
  if(persistence) {
    var _originalIsOnline = persistence.options.isOnline;
    var _isOffline = false;

    persistence.options.isOnline = function () {
  ...
  this.returnMobileBackend = function (name, config) {
    ...
    if (g.cordova) {
      this.platform = new CordovaPlatform(logger, utils);
      logger.info("The Cordova platform is set!");
    }
    else {
      this.platform = new BrowserPlatform(logger, utils);
  ...
  this.getMobileBackend = function(name) {
    ...
    if(_config.mobileBackends[name]){
      var backend = new MobileBackend(this, name, _config.mobileBackends[name], this.platform, utils, logger, persistence);
  ...
  function _initPersistenceConfiguration(config) {
    ...
    if(config.sync && config.syncExpress) {
      ...
      persistence.options.Module = persistence.requestHandler;
    } else {
      logger.debug('WARNING, missing synchronization configuration, switch caching off');
      persistence.options.off = true;
      return;
    }
 ...
 * @global
 */
function StorageCollection(data, userId, backend, utils, logger, platform) {
  var HEADERS = utils.HEADERS;
  ...
  this.getObjects = function(offset, limit, allObjects, successCallback, errorCallback) {
    ...
    if (storageCollection._userIsolated && allObjects) {
      url += url.indexOf("?") == -1 ? "?" : "&";
      url += "user=*";
    } else if(storageCollection._userIsolated && storageCollection.getUserId() != null){
      url += url.indexOf("?") == -1 ? "?" : "&";
   ...
   * @example mcs.MobileBackend.getStorage().StorageCollection.getObject(id,successCallback,errorCallback,objectType);
   ...
   *
   *
   * @example <caption>Example usage of mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",successCallback,errorCallback,"blob")</caption>
   *
   * @example mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",successCallback,errorCallback,"blob");
   * mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",
   ...
   *
   * @example mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",successCallback,errorCallback,"arraybuffer");
   * mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",
   * function(statusCode, data){
   * },
   * function(statusCode, data){
   ...
   *
   * @example mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",successCallback,errorCallback,"document");
   * mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",
   * function(statusCode, data){
   * },
   * function(statusCode, data){
   ...
   *
   * @example mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",successCallback,errorCallback,"text");
   * mcs.MobileBackend.getStorage().StorageCollection.getObject("00e39862-9652-458b-9a82-d1a66cf1a0c7",
   * function(statusCode, data){
   * },
   * function(statusCode, data){
...
function Storage(backend, utils, platform, logger) {
  ...
  this.getCollection = function(name, userId, successCallback, errorCallback) {
    ...
    return platform.invokeService({
      method: utils.HTTP_METHODS.GET,
      url: _backend.getPlatformUrl("storage/collections/" + utils.validateConfiguration(name)),
      headers: headers
    }).then(invokeServiceSuccess, invokeServiceError);

    function invokeServiceSuccess(response) {
      var collection = new StorageCollection(response.data, utils.validateConfiguration(userId), _backend, utils, logger, platform);
      if(successCallback) {
...
function SyncProcessor(backend, apiName, endpointPath, resolvingOfflineUpdate, utils, persistence) {
  ...
  this.getUri = function(response, url){
    ...
    if(location != null){
      ...
      if(obj && obj[persistence.mcs.URI_KEY]){
        var uri = obj[persistence.mcs.URI_KEY];
        delete obj[persistence.mcs.URI_KEY];
        return uri;
      } else {
...
function MobileEndpoint(synchronization, apiName, endpointPath, utils, platform) {
   ...
   */
  this.purge = function () {
    persistence.mcs.flush(_this._backend.getCustomCodeUrl(_this.apiName + "/" + _this.endpointPath));
  };
...
mcs.mobileBackendManager.platform = new BrowserPlatform(mcs._logger, mcs._utils);
